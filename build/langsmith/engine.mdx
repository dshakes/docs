...
Add scope conditions with the same [filter editor](/langsmith/filter-traces-in-application#create-and-apply-filters) used on the tracing project's **Tracing** tab, so it behaves identically. You can add one condition of each kind, up to two:
...
To add a condition, choose its kind from the field selector, fill in the values, then click **Add**. After you add one kind, the editor offers only the other; once both are set, it stops offering more. Each condition appears as a chip, for example `Run Name is chatbot` or `env is prod`. Click the **×** on a chip to remove that condition.
